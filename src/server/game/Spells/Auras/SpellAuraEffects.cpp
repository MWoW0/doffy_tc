...
void AuraEffect::CalculatePeriodic(Unit* caster, bool resetPeriodicTimer /*= true*/, bool load /*= false*/)
{
    ...
    bool l_IsHealSpell = GetAuraType() == SPELL_AURA_PERIODIC_HEAL || GetAuraType() == SPELL_AURA_OBS_MOD_HEALTH; ///< Heal spells to not be affected by haste
    ...
    if (m_amplitude || m_isPeriodic)
    {
        ...
        if (caster)
        {
            // Haste Cap doesn't let channeled scale with haste.
            /*
        ...
        //    else if ( m_spellInfo->HasAttribute(SPELL_ATTR5_HASTE_AFFECT_DURATION))
            if(!m_spellInfo->IsChanneled() && !l_IsHealSpell)
				m_amplitude = int32(m_amplitude * caster->GetFloatValue(UNIT_MOD_CAST_SPEED))/4;
            else if (l_IsHealSpell) ///< Heal spells have different formula now
